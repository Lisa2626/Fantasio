# Helper functions for continuum normalization fitting, work with interactive.py
...
def readFitsFile(observationName, choice='AB', trimMax=-1.):
    ...
    obsWl_order = np.full_like(obsI, np.nan)
    obsWl_order[~nan_positions_obsI] = obsWl[~nan_positions_obsI]

    obsI_order = np.full_like(obsI, np.nan)
    ...
    return obsWl_order, obsI_order, Wl


# Remove NAN value, useful for next calculations
def remove_nan_rows(obsWl_order, obsI_order):
        obsWl = []
        obsI = []
...
def fit_order(obsWl, obsI, Wl, obsWl_order, obsI_order, k, t):
...
def restore_params(order_num, k_var, t_var, sigma_above_var, sigma_below_var, num_iterations_var, fit_dict):
    params = fit_dict.get('parameters', [])
    if params:
        window_params = params[order_num]
        k_var.set(window_params.get('k', 3.0))
        t_var.set(window_params.get('t', 3.0))
        sigma_above_var.set(window_params.get('sigma_above', 3))
        sigma_below_var.set(window_params.get('sigma_below', 2.0))
        num_iterations_var.set(window_params.get('num_iterations', 2.0))
...
def interactive(fit_dict):
    ...
    k_var = IntVar(value=fit_dict.get('k', 3.0))  #degree of polynone is 5 by default, can be change
    t_var = IntVar(value=fit_dict.get('t', 3.0))
    sigma_above_var = DoubleVar(value=fit_dict.get('sigma_above', 3))
    sigma_below_var = DoubleVar(value=fit_dict.get('sigma_below', 2))
    num_iterations_var = IntVar(value=fit_dict.get('num_iterations', 2.0))
...
def save(observationName, fit_dict, choice='AB', output_directory=None):
    ...
    output_filename = os.path.splitext(os.path.basename(observationName))[0] + '_param_norm.fits'
